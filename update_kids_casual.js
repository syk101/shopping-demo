...
const imageDir = path.resolve(__dirname, 'frontend', 'public', 'kid_casual');
...
function updateImages() {
  ...
  db.all("SELECT id, name FROM products WHERE category = 'kids_CASUAL' ORDER BY id", (err, products) => {
    if (err) {
    ...
    if (products.length === 0) {
        console.log('No products found with category kids_CASUAL.');
        db.close();
        return;
    }
    ...
    products.forEach((product, index) => {
      ...
      const imagePath = `kid_casual/${images[imageIndex]}`;
