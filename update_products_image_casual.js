...
const imagesDir = path.join(__dirname, 'frontend', 'public', 'casual wear');
...
db.all("SELECT id FROM products WHERE category = 'men_CASUAL'", [], (err, rows) => {
    if (err) {
        console.error("Error reading products:", err.message);
        return;
    }

    console.log(`Found ${rows.length} men_CASUAL products in products table.`);
    ...
    rows.forEach(row => {
        ...
        const imagePath = `casual wear/${randomImage}`;
